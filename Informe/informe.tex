...
\subsection*{Funcionamiento general del modelo}
El modelo se apoya fuertemente en la temporalidad y en las características contextuales de los datos. La idea general es aprovechar la idea de que, si el usuario está en un momento de indecisión (realizando muchos skips seguidos), es más probable que también saltee esta observación. Por el contrario, si viene en una seguidilla de escuchar canciones completas, es más probable que también escuche completa la actual.

Para realizar una predicción para subir a la plataforma, se lleva a cabo el siguiente proceso:
\begin{itemize}
	\item Se entrena el modelo de XGBoost con la totalidad de los datos disponibles. A esos datos, se les aplica el procesamiento llevado a cabo por la función \texttt{procesar\_df()}.
	\item Se calculan las mismas columnas sobre los datos de submission, como se explicó en la sección "Variables creadas".
	\item Se utiliza el modelo para predecir las probabilidades de que la reproducción sea efectivamente skippeada.
\end{itemize}

Cabe mencionar que nos permitimos utilizar la variable \texttt{TARGET} para calcular \texttt{racha\_skips\_prev}, dado que en un contexto de la vida real serían datos disponibles. Al yo recibir una observación nueva, tengo la información de si las anteriores fueron skips o no. Por lo tanto es información que puedo usar para mi modelo.

Esta dinámica es la que se busca replicar al insertar las observaciones de submission con las de entrenamiento para calcular \texttt{racha\_skips\_prev}, insertarlas cronológicamente donde iban y calcular la racha desde ahí.
